import { photos, albums } from './data.js';
import { OneDriveAdapter } from './onedrive-adapter.js';

const app = document.querySelector('#app');
const state = { route:'home', selected:null, query:'', signedIn:false, upload:null };
  ...
  app.innerHTML = `
    <div class="shell">
      <aside class="desktop-sidebar">
        ${brand()}
        <nav>${desktopNav()}</nav>
        <div class="storage-card"><div>${icon('cloud')}<span>OneDrive Storage</span></div><div class="storage-track"><span style="width:13%"></span></div><small>128 GB of 1 TB used</small></div>
...
function topbar(){return `<header class="topbar"><div class="mobile-brand">${brand()}</div><div class="topbar-search">${icon('search')}<input id="globalSearch" value="${state.query}" placeholder="Search your photos…"/></div><div class="sync-pill">${icon('cloud')}<span>Synced<small>Up to date</small></span></div><button class="avatar">J</button></header>`}
...
function routeView(){
 ...
 if(state.route==='favorites') return libraryView(photos.filter(p=>p.favorite),'Favorites');
...
function homeView(){
 return `<section class="page home-page">
   <div class="hero-row"><div><p class="eyebrow">PRIVATE PHOTO LIBRARY</p><h1>Good evening, JAT.</h1><p>${photos.length * 81 - 1} photos · Synced with OneDrive</p></div><div class="hero-badge">${icon('lock')} Private by design</div></div>
...
function albumsView(){return `<section class="page"><div class="page-title"><div><p class="eyebrow">COLLECTIONS</p><h1>Albums</h1></div><button class="round-action">+</button></div><div class="segmented"><button class="selected">My Albums</button><button>Shared</button><button>Favorites</button></div><div class="album-list">${albums.map(a=>{const p=photos.find(x=>x.id===a.cover);return `<button class="album-row"><img src="${p.src}" alt=""/><span><strong>${a.name}</strong><small>${a.count} photos</small></span><b>•••</b></button>`}).join('')}</div></section>`}
...
function searchView(){
 const q=state.query.toLowerCase().trim(); const filtered=photos.filter(p=>!q || [p.title,p.album,p.camera,p.type,p.location,...p.tags].join(' ').toLowerCase().includes(q));
...
function moreView(){return `<section class="page narrow"><div class="page-title"><div><p class="eyebrow">JATCAM</p><h1>More</h1></div></div><div class="feature-cards"><article><span>${icon('cloud')}</span><div><strong>OneDrive</strong><p>Private cloud storage for originals, albums and app metadata.</p></div></article><article><span>${icon('camera')}</span><div><strong>Canon friendly</strong><p>Designed around the T7i → phone/SD card → JATcam workflow.</p></div></article><article><span>${icon('lock')}</span><div><strong>Private by design</strong><p>Your photography library remains yours.</p></div></article></div></section>`}
...
function bindEvents(){
 ...
 document.querySelectorAll('[data-photo]').forEach(b=>b.onclick=()=>{state.selected=photos.find(p=>p.id===b.dataset.photo);render();});
...
async function upload(fileList){const files=[...fileList];if(!files.length)return;state.upload={complete:0,total:files.length,name:''};render();await OneDriveAdapter.uploadFiles(files,p=>{state.upload=p;render();});setTimeout(()=>{state.upload=null;render();},800);}
